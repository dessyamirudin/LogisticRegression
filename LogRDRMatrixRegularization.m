...
for i=1:10
    ...
    while diff>=eps
        ...
        cost=(-1/m)*sum(A+B)+(lambda/m)*sum(thetha(1:(end-1),i).^2);%cost function with regularization
        
        %update theta
        
        minusizer=(1/m).*(hthetha-Y(:,i))'*X;
        thetha(1:(end-1),i)=(thetha(1:(end-1),i)-alpha*(lambda/m).*thetha(1:(end-1),i))-alpha.*minusizer(1:(end-1))';
        thetha(end)=thetha(end)-alpha*minusizer(end);
...
csvwrite('submit.csv',Ysubmitfinal)